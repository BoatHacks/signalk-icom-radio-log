...
// (forensic — exact captured bytes, undecoded) and decodes them to a
// playable WAV. Both are written to disk once, at capture time
// (index.js's finishTransmission) — a real .wav on disk plays reliably
// (seekable, no surprises for <audio>), unlike an earlier version of this
// plugin that decoded to WAV on every /audio request instead of storing
// one.
//
...
// The raw and decoded files for one transmission share a basename
// (`<startTs>-ch<channelNr>`), differing only in extension — lets
// index.js and lib/retention.js find/delete the paired file from either
// path without a second DB column.
function rawPathToWavPath (rawPath) {
  return rawPath.replace(/\.raw$/, '.wav')
}
