...
class RadioClient extends EventEmitter {
  constructor ({ bindAddress, busyDebounceMs = 200 } = {}) {
    super()
    this.bindAddress = bindAddress
    this.radio = { ip: null, port: null }
    this.signedIn = false
    this.busy = false
    this._activeChannel = null // channelNr of the transmission currently open, or null
  ...
  _onServerCMessage (msg) {
    ...
    if (status.busy) {
      if (this._activeChannel === null) {
        this._activeChannel = status.channelNr
        this.busy = true
        this.emit('tx-start', { channelNr: status.channelNr, startTs: Date.now() })
      } else if (status.channelNr === this._activeChannel && this._pendingEnd) {
        clearTimeout(this._pendingEnd)
        this._pendingEnd = null
      }
